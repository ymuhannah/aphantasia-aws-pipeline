...
sns_topic_arn = 'arn:aws:sns:us-east-1:888853202619:egift-card-compensation-parquet'

# Download files from S3
obj1 = s3.get_object(Bucket=bucket1, Key=egift_cards_file)
egift_df = pd.read_parquet(BytesIO(obj1['Body'].read()), engine='pyarrow')

obj2 = s3.get_object(Bucket=bucket2, Key=participants_data_file)
participants_df = pd.read_parquet(BytesIO(obj2['Body'].read()), engine='pyarrow')
...
for index, participant in participants_df[participants_df['egift-card-sent'] == 0].iterrows():
    ...
    if total_amount == gift_card_value:
        ...
        # Send notification
        response = send_notification(email_subject, notification_message)
        if response:
            print(f"Notification sent for {email_address}")
...
participants_df.to_parquet(participants_buffer, index=False, engine='pyarrow')
participants_buffer.seek(0)

egift_buffer = BytesIO()
egift_df.to_parquet(egift_buffer, index=False, engine='pyarrow')
egift_buffer.seek(0)
